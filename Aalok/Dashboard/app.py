...
# component iframe (same-origin srcdoc) and decorates the PARENT page:
# particle network background, trailing ghost cursor, magnetic buttons,
# cursor spotlight, decode-glitch hero title, terminal-typed subtitle, and
# scroll parallax. Runs once per browser page load (flag on window.parent),
# fully disabled for prefers-reduced-motion; cursor FX skipped on touch.
_FX_JS = """
<script>
(function () {
  ...
  if (P.__idsFx) return;
  P.__idsFx = true;
  /* NOTE: deliberately NOT gating on prefers-reduced-motion — Windows
     machines with "Animation effects" off report it, which silently killed
     the whole demo. Cursor FX still require a fine pointer. */
  const fine = P.matchMedia('(pointer: fine)').matches;

  const appView = D.querySelector('[data-testid="stAppViewContainer"]') || D.body;
  const mouse = { x: -9999, y: -9999 };
  D.addEventListener('mousemove', e => { mouse.x = e.clientX; mouse.y = e.clientY; }, { passive: true });

  /* ── Interactive particle network: warm nodes, links, cursor repulsion ── */
  ...
  let W, H;
  const resize = () => { W = cv.width = P.innerWidth; H = cv.height = P.innerHeight; };
  resize();
  P.addEventListener('resize', resize);
  const N = 30, pts = [];
  for (let i = 0; i < N; i++) {
    pts.push({
      x: Math.random() * P.innerWidth, y: Math.random() * P.innerHeight,
      vx: (Math.random() - 0.5) * 0.35, vy: (Math.random() - 0.5) * 0.35
    });
  }
  const LINK2 = 150 * 150, REPEL2 = 130 * 130;
  let _netLast = 0;
  function netStep(ts) {
    /* Cap to ~30fps — halves canvas/CPU cost vs uncapped rAF. */
    if (ts - _netLast < 33) { P.requestAnimationFrame(netStep); return; }
    _netLast = ts;
    ...
    for (const p of pts) {
      ...
      cx.fillStyle = 'rgba(217,119,87,0.55)';
      cx.fill();
    }
    for (let i = 0; i < N; i++) for (let j = i + 1; j < N; j++) {
      ...
      if (d2 < LINK2) {
        ...
        cx.strokeStyle = 'rgba(217,119,87,' + (0.15 * (1 - d2 / LINK2)).toFixed(3) + ')';
        cx.lineWidth = 1;
        cx.stroke();
      }
    }
    P.requestAnimationFrame(netStep);
  }
  P.requestAnimationFrame(netStep);

  /* ── Spotlight: soft light layer riding ABOVE content (screen blend) ── */
  ...
  D.addEventListener('mousemove', () => {
    if (spotRaf) return;
    spotRaf = P.requestAnimationFrame(() => {
      spotRaf = null;
      spot.style.background =
        'radial-gradient(600px at ' + mouse.x + 'px ' + mouse.y + 'px,' +
        ' rgba(250,249,245,0.05), transparent 65%)';
    });
  }, { passive: true });

  /* ── Scroll parallax: particle field drifts slower than the page ── */
  D.addEventListener('scroll', e => {
    const y = (e.target && e.target.scrollTop) || 0;
    cv.style.transform = 'translateY(' + (-y * 0.06) + 'px)';
  }, true);

  /* ── Trailing ghost cursor: terracotta dot + easing ring ── */
  if (fine) {
    const dot = D.createElement('div'), ring = D.createElement('div');
    Object.assign(dot.style, {
      position: 'fixed', width: '6px', height: '6px', borderRadius: '50%',
      background: '#D97757', pointerEvents: 'none', zIndex: '2147483647',
      transform: 'translate(-50%,-50%)', transition: 'opacity .25s', opacity: '0'
    });
    Object.assign(ring.style, {
      position: 'fixed', width: '30px', height: '30px', borderRadius: '50%',
      border: '1.5px solid rgba(250,249,245,0.35)', pointerEvents: 'none',
      zIndex: '2147483646', transform: 'translate(-50%,-50%)',
      transition: 'opacity .25s, border-color .2s, width .2s, height .2s, background .2s',
      opacity: '0', background: 'transparent'
    });
    D.body.append(dot, ring);
    let dx = -9999, dy = -9999, rx = -9999, ry = -9999, seen = false;
    D.addEventListener('mousemove', () => {
      if (!seen) { seen = true; dot.style.opacity = '1'; ring.style.opacity = '1'; }
    }, { passive: true });
    D.documentElement.addEventListener('mouseleave', () => {
      seen = false; dot.style.opacity = '0'; ring.style.opacity = '0';
    });
    /* ring swells + warms over anything clickable */
    D.addEventListener('mouseover', e => {
      const hot = e.target.closest &&
        e.target.closest('button, a, [role="tab"], input, select, label, [role="checkbox"]');
      if (hot) {
        ring.style.width = '44px'; ring.style.height = '44px';
        ring.style.borderColor = '#D97757';
        ring.style.background = 'rgba(217,119,87,0.08)';
      } else {
        ring.style.width = '30px'; ring.style.height = '30px';
        ring.style.borderColor = 'rgba(250,249,245,0.35)';
        ring.style.background = 'transparent';
      }
    }, { passive: true });
    function cursorStep() {
      dx += (mouse.x - dx) * 0.5;  dy += (mouse.y - dy) * 0.5;
      rx += (mouse.x - rx) * 0.16; ry += (mouse.y - ry) * 0.16;
      dot.style.left = dx + 'px'; dot.style.top = dy + 'px';
      ring.style.left = rx + 'px'; ring.style.top = ry + 'px';
      P.requestAnimationFrame(cursorStep);
    }
    cursorStep();
  }

  /* ── Magnetic buttons: pills lean toward the cursor inside 90px ── */
  if (fine) {
    let btns = [];
    const requery = () => {
      btns = Array.from(D.querySelectorAll('.stButton > button, .stDownloadButton > button'));
    };
    requery();
    P.setInterval(requery, 600);
    let magRaf = null;
    D.addEventListener('mousemove', () => {
      if (magRaf) return;
      magRaf = P.requestAnimationFrame(() => {
        magRaf = null;
        for (const b of btns) {
          const r = b.getBoundingClientRect();
          if (!r.width) continue;
          const cxr = r.left + r.width / 2, cyr = r.top + r.height / 2;
          const ddx = mouse.x - cxr, ddy = mouse.y - cyr;
          const dist = Math.hypot(ddx, ddy);
          if (dist < 90) {
            const pull = (1 - dist / 90) * 7;
            b.style.transform = 'translate(' + (ddx / dist * pull).toFixed(1) + 'px,' +
                                               (ddy / dist * pull).toFixed(1) + 'px)';
...
"""
# Heavy visual layer (particle field, cursor FX, magnetic buttons) is the main
# source of UI lag on weaker hardware. Let the user switch it off; default keeps
# the full look. Declared here so the gate runs before the FX iframe injects.
_lite_mode = st.sidebar.checkbox(
    "Reduce animations (fixes lag)",
    value=False,
    key="lite_mode",
    help="Turns off the background particle field, cursor effects and magnetic "
         "buttons. Enable this if the dashboard feels slow.",
)
if not _lite_mode:
...
with tab6:
    ...
    _active = [c for c in ("email", "discord", "slack")
               if _ncfg.get(c, {}).get("enabled")]
    st.caption(
        f"Editing **{_nsrc}** — file is gitignored (holds plaintext SMTP creds). "
        f"Active channels: **{', '.join(_active) if _active else 'none'}**. "
